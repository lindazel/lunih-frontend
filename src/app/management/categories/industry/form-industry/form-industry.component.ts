...
@Component({
  ...
  styleUrls: ['./form-industry.component.scss']
})
export class FormIndustryComponent {
  ...
  onSubmit() {
    if (this.form.valid) {
      if (this.modalData.action === SystemConstant.ACTION.EDIT) {
        this.industryService.update(this.form.value, this.modalData.data?.id!)
        .subscribe(() => {
        ...
        this.industryService.create(this.form.value)
        .subscribe(() => {
          this.closeModal.emit(true);
